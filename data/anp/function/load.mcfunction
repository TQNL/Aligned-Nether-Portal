tellraw @a {"text": "Loaded Aligned Nether Portal","color": "#00FF00"}
scoreboard objectives add nether_portal_pos_per_nether_X dummy
scoreboard objectives add nether_portal_pos_per_nether_Z dummy
scoreboard objectives add nether_portal_math dummy
scoreboard objectives add nether_portal_pos_per_nether_Y dummy
scoreboard objectives add nether_portal_clone_pos_X dummy
scoreboard objectives add nether_portal_clone_pos_per_nether_Y dummy
scoreboard objectives add nether_portal_clone_pos_Z dummy
scoreboard players set #8 nether_portal_math 8
scoreboard objectives add toggle_vanilla_aligned_nether_portals_for_you trigger
scoreboard objectives add ov_nether_portal_pos_X dummy
scoreboard objectives add ov_nether_portal_pos_Y dummy
scoreboard objectives add ov_nether_portal_pos_Z dummy
scoreboard objectives add anp_player_still_time dummy
